...
struct stats_t {
  ...
  void dump(std::map<std::string, ssize_t> &f) const {
    f["item"] = items;
    f["bytes"] = bytes;
  }
...
public:
  ...
  // void dump(ceph::Formatter *f, stats_t *ptotal=0) const; // remove by simon
  void dump(std::map<std::string, ssize_t> &f, stats_t *ptotal=0) const; // remove by simon
};
  // remove by simon
  // void dump(ceph::Formatter *f);
  void dump(std::map<std::string, ssize_t> &f);
