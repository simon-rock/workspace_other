...
class interval_set {
  ...
  */
  ...
  void insert(T start, T len, T *pstart=0, T *plen=0) {
    ...
    // assert(len > 0);  // remove by simon
    cout << "assert(len > 0);" << endl;
    _size += len;
	...
	if (pstart)
	  *pstart = p->first;
	  ...
	  T psecond = p->second;
          ...
          // assert(p->first > start+len);  // remove by simon
	  cout << "assert(p->first > start+len)" << endl;
	  if (pstart)
	    *pstart = start;
	  if (plen)
	    *plen = len;
  ...
  void erase(iterator &i) {
    _size -= i.get_len();
    // assert(_size >= 0); // remove by simon
    cout << "assert(_size >= 0)" << endl;
    m.erase(i._iter);
  }
